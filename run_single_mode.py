...
for data_name in data_list:
    if data_name == "cora" or data_name == "citeseer" or data_name == "pubmed":
        ...
        for model_name in model_list:
            for loss_oriented in loss_oriented_list:
                ...
                for run in run_times:
                    model = Single_model(model_name, loss_oriented, data.x.shape[1], hid_dim, number_class, data, verbose=0)
                    model.fit()
        ...
        data = pyg_dataset(dataset_name=data_name, dataset_spilt=[0.4,0.29,0.3], anomaly_type="syn", anomaly_ratio=0.2).dataset
        for model_name in model_list:
            for loss_oriented in loss_oriented_list:
                test_auc_list = []
                best_auc_list = []
        ...
        for model_name in model_list:
            for loss_oriented in loss_oriented_list:
                ...
                for run in run_times:
                    ...
                    best_auc_list.append(best_auc)
                    test_auc_list.append(test_auc)
                print (f"Single Mode: model name {model_name}; dataset {data_name}; loss_oriented {loss_oriented}; anomaly type organic; test auc mean {np.array(test_auc_list).mean()}; test auc std {np.array(test_auc_list).std()}; best val auc mean {np.array(best_auc_list).mean()}")
                with open("result/singlemode_for_all_dataset.txt",'a') as f:
                    f.write(f"Single Mode: model name {model_name}; dataset {data_name}; loss_oriented {loss_oriented}; anomaly type organic; test auc mean {np.array(test_auc_list).mean()}; test auc std {np.array(test_auc_list).std()}; best val auc mean {np.array(best_auc_list).mean()}\n")            
# with open("result/singlemode_for_all_dataset.txt",'a') as f:
