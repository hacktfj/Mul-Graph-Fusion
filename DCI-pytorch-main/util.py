...
def load_data(datasets, num_folds):
    # load the adjacency
    fold_name = "DCI-pytorch-main"
    adj = np.loadtxt(f'./{fold_name}/data/'+datasets+'.txt')
    ...
    # load the user label
    label = np.loadtxt(f'./{fold_name}/data/'+datasets+'_label.txt')
    y = label[:, 1]
    ...
    feats = np.load(f'./{fold_name}/features/'+datasets+'_feature64.npy')
